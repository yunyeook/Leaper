...
public class SparkKeywordTrendService extends SparkBaseService {
   ...
   * @param contentData 오늘의 콘텐츠 데이터 (캐시됨)
   */
  public void generateDailyKeywordTrend(String platformType, LocalDate targetDate, Dataset<Row> contentData) {
    try {
      // 1. 오늘 크롤링된 콘텐츠 데이터 (파라미터로 받음)
  ...
  private void saveKeywordTrendToS3(String platformType, List<Row> topKeywords, LocalDate targetDate) {
    try {
      ...
      String s3Path = String.format("processed_data/json/%s/daily_keyword_trend/%s/%s",
