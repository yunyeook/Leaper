...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    List<ChatRoom> findByInfluencerIdAndNotDeleted(@Param("influencerId") Long influencerId);
    ...
    @Query("SELECT c FROM ChatRoom c WHERE c.advertiserId = :advertiserId AND c.advertiserDeleted = false " +
           "ORDER BY c.createdAt DESC")
    List<ChatRoom> findByAdvertiserIdAndNotDeleted(@Param("advertiserId") Long advertiserId);
    ...
    @Query("SELECT c FROM ChatRoom c WHERE " +
           ...
           "ORDER BY c.createdAt DESC")
    List<ChatRoom> findByUserIdAndNotDeleted(@Param("userId") Long userId);

    @Modifying
    @Query("UPDATE ChatRoom c SET c.influencerLastSeen = :lastSeen WHERE c.id = :chatRoomId")
    void updateInfluencerLastSeen(@Param("chatRoomId") Long chatRoomId, @Param("lastSeen") LocalDateTime lastSeen);

    @Modifying
    @Query("UPDATE ChatRoom c SET c.advertiserLastSeen = :lastSeen WHERE c.id = :chatRoomId")
    void updateAdvertiserLastSeen(@Param("chatRoomId") Long chatRoomId, @Param("lastSeen") LocalDateTime lastSeen);

    @Modifying
    @Query("UPDATE ChatRoom c SET c.influencerDeleted = true, c.influencerDeletedAt = :deletedAt WHERE c.id = :chatRoomId")
    void deleteByInfluencer(@Param("chatRoomId") Long chatRoomId, @Param("deletedAt") LocalDateTime deletedAt);

    @Modifying
    @Query("UPDATE ChatRoom c SET c.advertiserDeleted = true, c.advertiserDeletedAt = :deletedAt WHERE c.id = :chatRoomId")
    void deleteByAdvertiser(@Param("chatRoomId") Long chatRoomId, @Param("deletedAt") LocalDateTime deletedAt);
    ...
    void restoreInfluencer(@Param("chatRoomId") Long chatRoomId);
    ...
    void restoreAdvertiser(@Param("chatRoomId") Long chatRoomId);
}